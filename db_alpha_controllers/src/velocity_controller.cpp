...
#include "db_alpha_controllers/velocity_controller.h"


// Constructur
VelocityController::VelocityController() : node_handle(""), priv_node_handle("~"), has_joint_state(false) 
{
...
bool VelocityController::initWorkbench(const std::string port_name, const uint32_t baud_rate) 
{
	bool result = false;
	const char* log;
...
bool VelocityController::getDynamixelsInfo(const std::string yaml_file)
{
	YAML::Node dxl_node;
...
bool VelocityController::loadDynamixels()
{
	bool result = false;
	const char* log;
	int motor_cnt = 0;
...
void VelocityController::initMsg()
{
    printf("--------------------------------------------------------------------------\n");
    printf("\n"
           "  ____                              _          _  \n"
...
bool VelocityController::initDynamixels() 
{
...
bool VelocityController::initControlItems()
{
	bool result = false;
	const char* log = NULL;
	...
	const ControlItem* goal_velocity = dxl_wb->getItemInfo(dxl_num, "Goal_Velocity");
...
bool VelocityController::initSDKHandlers()
{
	bool result = false;
	const char* log = NULL;
...
void VelocityController::initPublisher()
{
...
void VelocityController::initSubscriber()
{
	goal_joint_state_sub = priv_node_handle.subscribe("hexapod_state_commands", 100, &VelocityController::onJointStateGoal, this);
}

void VelocityController::initServer()
{
	dynamixel_command_server = priv_node_handle.advertiseService("dynamixel_request_commands", &VelocityController::dynamixelCommandMsgCallback, this);
}

void VelocityController::onJointStateGoal(const sensor_msgs::JointState& msg)
{
	goal_state = msg;
...
void VelocityController::readCallback(const ros::TimerEvent&)
{
	bool result = false;
	const char* log = NULL;
...
void VelocityController::writeCallback(const ros::TimerEvent& t)
{
	...
	int32_t dynamixel_velocity[dynamixel.size()];
	...
	for (uint8_t index = 0; index < id_cnt; index++)
	{ 
		dynamixel_velocity[index] = dxl_wb->convertVelocity2Value(id_array[index], goal_state.velocity[index]);
	}

	result = dxl_wb->syncWrite(SYNC_WRITE_HANDLER_FOR_GOAL_VELOCITY, id_array, id_cnt, dynamixel_velocity, 1, &log);
	if (result == false) ROS_ERROR("%s", log);
	
	has_joint_state = false;
}

void VelocityController::publishCallback(const ros::TimerEvent&)
{
...
bool VelocityController::dynamixelCommandMsgCallback(dynamixel_workbench_msgs::DynamixelCommand::Request &req, dynamixel_workbench_msgs::DynamixelCommand::Response &res)
{
	bool result = false;
	const char* log;
...
int main(int argc, char** argv)
{
	...
	result = velocity_controller.initWorkbench(port_name, baud_rate);
	if (result == false)
	{
	...
	velocity_controller.initMsg();

	result = velocity_controller.getDynamixelsInfo(yaml_file);
	if (result == false)
	{
	...
	result = velocity_controller.loadDynamixels();
	if (result == false)
	{
	...
	result = velocity_controller.initDynamixels();
	if (result == false)
	{
	...
	result = velocity_controller.initControlItems();
	if (result == false)
	{
	...
	result = velocity_controller.initSDKHandlers();
	if (result == false)
	{
	...
	velocity_controller.initPublisher();
	velocity_controller.initSubscriber();
	velocity_controller.initServer();
	
	ros::Timer read_timer = node_handle.createTimer(ros::Duration(velocity_controller.getReadPeriod()), &VelocityController::readCallback, &velocity_controller);
	ros::Timer write_timer = node_handle.createTimer(ros::Duration(velocity_controller.getWritePeriod()), &VelocityController::writeCallback, &velocity_controller);
	ros::Timer publish_timer = node_handle.createTimer(ros::Duration(velocity_controller.getPublishPeriod()), &VelocityController::publishCallback, &velocity_controller);
